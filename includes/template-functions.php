<?php
/**
 * Functions which enhance the theme by hooking into WordPress
 *
 * @package stier
 */

/**
 * Adds custom classes to the array of body classes.
 *
 * @param array $classes Classes for the body element.
 * @return array
 */

function stier_body_classes( $classes ) {
	// Adds a class of hfeed to non-singular pages.
	if ( ! is_singular() ) {
		$classes[] = 'hfeed';
	}
	return $classes;
}
add_filter( 'body_class', 'stier_body_classes' );

// Add a pingback url auto-discovery header for single posts, pages, or attachments.

function stier_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'stier_pingback_header' );


// Login Screen

add_filter( 'login_headerurl', 'my_custom_login_url' );
function my_custom_login_url($url) {
    return '/';
}

function get_custom_logo_url() {
    $custom_logo_id = get_theme_mod('custom_logo');
    $logo = wp_get_attachment_image_src($custom_logo_id, 'full');

    if (has_custom_logo()) {
        return $logo[0];
    } else {
        return ''; // Return an empty string or a default logo URL if no custom logo is set
    }
}

function custom_login_logo() {
    $logo_url = get_custom_logo_url();
    if ($logo_url != '') {
        ?>
        <style type="text/css">
            #login h1 a, .login h1 a {
                background-image: url(<?php echo esc_url($logo_url); ?>);

            }
        </style>
        <?php
    }
}
add_action('login_enqueue_scripts', 'custom_login_logo');


// Make ACF Imposible to deactivate

add_filter('user_has_cap', 'prevent_plugin_deactivation', 10, 3);

function prevent_plugin_deactivation($allcaps, $cap, $args) {
    $plugin_file = 'advanced-custom-fields-pro/acf.php';

    if (isset($args[2]) && $args[2] == $plugin_file) {
        if ($args[0] == 'deactivate_plugin') {
            $allcaps[$cap[0]] = false;
        }
    }
    return $allcaps;
}


// Admin footer modification

function dashboard_footer_admin () {
    echo '<span id="footer-thankyou">Thank you for developing with <a href="https://stierdev.com/" target="_blank">S Tier Dev</a>. Powered by <a href="https://wordpress.org/" target="_blank">WordPress</a>.</span> ';
}

add_filter('admin_footer_text', 'dashboard_footer_admin');

// Settings pages

if( function_exists('acf_add_options_page') ) {

    acf_add_options_page(array(
        'page_title'    => 'Website Settings',
        'menu_title'    => 'Site Settings',
        'menu_slug'     => 'site-general-settings',
        'capability'    => 'edit_posts',
        'redirect'      => false,
		'position' => '2.69',
		'icon_url'          => 'data:image/svg+xml;base64,'
    ));

	acf_add_options_sub_page(array(
		'page_title'    => 'Footer Settings',
		'menu_title'    => 'Footer',
		'parent_slug'   => 'site-general-settings',
	));


	acf_add_options_sub_page(array(
		'page_title'    => 'External Scripts',
		'menu_title'    => 'External Scripts',
		'parent_slug'   => 'site-general-settings',
	));

	acf_add_options_sub_page(array(
		'page_title'    => 'Grants & Awards Archive',
		'menu_title'    => 'Archive',
		'parent_slug'   => 'edit.php?post_type=grants-awards',
	));

}


// Block Category and placement of it

add_filter('block_categories_all', 'stier_block_category');

function stier_block_category($cats)
{

	// create a new array element with anything as its index
	$new = array(
		'cpsf_block_cat' => array(
			'slug'  => 'cpsf',
			'title' => 'CPSF'
		)
	);
	$new2 = array(
		'cpsf_grant' => array(
			'slug'  => 'cpsf-grant',
			'title' => 'CPSF Grant/Awards/Scholarships'
		)
	);
	$position = 0.1; //

	$cats = array_slice($cats, 0, $position, true) + $new + $new2  + array_slice($cats, $position, null, true);

	// reset array indexes
	$cats = array_values($cats);

	return $cats;
}

// Get campaign for GiveWP plugin

function getCampaign($campaignID)
{
	global $wpdb;

	$table = $wpdb->prefix . 'give_campaigns';
	$sql = $wpdb->prepare("SELECT * FROM $table WHERE id = %d", $campaignID);

	return  $wpdb->get_row($sql);
}

// Get all campaign for GiveWP plugin

function getCampaigns()
{
	global $wpdb;
	$table = $wpdb->prefix . 'give_campaigns';
	$sql = $wpdb->prepare("SELECT * FROM $table");
	return  $wpdb->get_results($sql);
}

function getCampaignsForms($campaignID)
{
	global $wpdb;
	$table = $wpdb->prefix . 'give_campaign_forms';
	$sql = $wpdb->prepare("SELECT `form_id` FROM $table WHERE `campaign_id` = %d", $campaignID);
	return  $wpdb->get_results($sql);
}

function getCampaignsFormsMeta($formID, $meta_key)
{
	global $wpdb;
	$value  = '';
	$table = $wpdb->prefix . 'give_formmeta';
	$sql = $wpdb->prepare("SELECT `meta_value` FROM $table WHERE `form_id` = %d AND `meta_key` = %s", $formID, $meta_key);

	$row = $wpdb->get_row($sql);

	if ($row && property_exists($row, 'meta_value')) {
		$value = $row->meta_value;
	}

	return $value;
}

function getDonorsCount(array $formIds)
{
	global $wpdb;

	if (empty($formIds)) {
		return 0;
	}

	$placeholders = implode(',', array_fill(0, count($formIds), '%d'));

	$sql = "
		SELECT COUNT(DISTINCT meta_value) as donor_count
		FROM {$wpdb->prefix}give_donationmeta
		WHERE meta_key = '_give_payment_donor_id'
		AND donation_id IN (
			SELECT donation_id
			FROM {$wpdb->prefix}give_donationmeta
			WHERE meta_key = '_give_payment_form_id'
			AND meta_value IN ($placeholders)
		)
	";

	$query = $wpdb->prepare($sql, ...$formIds);
	$result = $wpdb->get_var($query);

	return (int) $result;
}

function progress_bar($type, $goal, $amount, $donations, $donors)
{
	$progress = 0;

	if ($goal <= 0) {
		return 0;
	}

	if ($type === 'amount') {
		$progress = $amount / $goal;
	} elseif ($type === 'donations') {
		$progress = $donations / $goal;
	} elseif ($type === 'donors') {
		$progress = $donors / $goal;
	}

	return min(round($progress * 100, 2), 100);
}


function format_goal($type, $goal)
{
	if ($type === 'amount') {
		printf("$%s amount", number_format($goal, 2));
	} elseif ($type === 'donations') {
		printf("%s donations", $goal);
	} elseif ($type === 'donors') {
		printf("%s donors", $goal);
	}
}

function format_current($type, $amount, $donations, $donors)
{
	if ($type === 'amount') {
		printf("$%s amount", number_format($amount, 2));
	} elseif ($type === 'donations') {
		printf("%s donations", $donations);
	} elseif ($type === 'donors') {
		printf("%s donors", $donors);
	}
}


function acf_populate_campaigns($field)
{


	$field['choices'] = array();
	$campaigns = getCampaigns();

	if (is_array($campaigns)) {
		foreach ($campaigns as $campaign) {
			$field['choices'][$campaign->id] = $campaign->campaign_title;
		}
	}

	return $field;
}

add_filter('acf/load_field/name=featured_campaign',  'acf_populate_campaigns');

// Blocks

add_action( 'init', 'register_acf_blocks' );
function register_acf_blocks() {
	register_block_type( __DIR__ . '/../blocks/home-hero' );
	register_block_type( __DIR__ . '/../blocks/info-boxes' );
	register_block_type( __DIR__ . '/../blocks/testimonial' );
	register_block_type( __DIR__ . '/../blocks/featured-event' );
	register_block_type( __DIR__ . '/../blocks/counters' );
	register_block_type( __DIR__ . '/../blocks/featured-campaign' );
	register_block_type( __DIR__ . '/../blocks/instagram' );
	register_block_type( __DIR__ . '/../blocks/cta' );
	register_block_type( __DIR__ . '/../blocks/inner-hero-alt' );
	register_block_type( __DIR__ . '/../blocks/advanced-section' );
	register_block_type( __DIR__ . '/../blocks/ways-to-give' );
	register_block_type( __DIR__ . '/../blocks/logos' );

	register_block_type( __DIR__ . '/../blocks/grants-awards/grant-hero' );
	register_block_type( __DIR__ . '/../blocks/grants-awards/grant-anchors' );
	register_block_type( __DIR__ . '/../blocks/grants-awards/grant-description' );
	register_block_type( __DIR__ . '/../blocks/grants-awards/grant-report' );
	register_block_type( __DIR__ . '/../blocks/grants-awards/grant-resources' );
	register_block_type( __DIR__ . '/../blocks/grants-awards/nominate' );

	register_block_type( __DIR__ . '/../blocks/accordion' );
	register_block_type( __DIR__ . '/../blocks/tabs' );
	register_block_type( __DIR__ . '/../blocks/info-boxes' );
	register_block_type( __DIR__ . '/../blocks/basic-section' );
	register_block_type( __DIR__ . '/../blocks/contact' );
	register_block_type( __DIR__ . '/../blocks/carousel' );
}
